...
export default function TaskCreateMulti() {
  const [repeat, setRepeat] = useState(false);
  return (
    <PhoneFrame bottomNav={parentTabs}>
      <div className={styles.screen}>
        <Link to="/parent/tasks-multi" className={styles.backLink}>
          ← Назад к заданиям
        </Link>
        <Section title="Создание задачи" subtitle="Выберите ребенка и заполните данные.">
          <div className={styles.formGrid}>
            <label className={styles.selectLabel}>
              <span className={styles.label}>Ребенок</span>
              <select className={styles.select} defaultValue="0">
                {mockChildren.map((c) => (
        ...
        <Button to="/parent/tasks-multi">Создать</Button>
      </div>
    </PhoneFrame>
  );
}
